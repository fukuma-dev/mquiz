<template>
  <v-container>
    ...
    <div v-if="!isStart">
      <v-btn @click="quizStart" block text outlined height="100px" class="start-btn">開始しますか？</v-btn>
    </div>
    <div v-if="isStart">
      <h2>プレイリスト名</h2>
      <h3>第{{ n + 1 }}問</h3>
      <div>
        ...
        <div v-if="!answer">
          ...
          <v-btn @click="switchPage" block large text outlined height="50px">答えを見る</v-btn>
        </div>
      </div>
      <div v-if="answer">
       <h4>答え</h4>
        <div class="text-center my-4">
          ...
          <v-btn v-if="trackExists(results, n)" @click="next" block large text outlined height="50px">次の問題へ</v-btn>
          <v-btn v-else to="/" block large text outlined height="50px">終了</v-btn>
        </div>
      </div>
    </div>
  </v-container>
